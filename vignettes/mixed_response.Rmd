...
In terms of the GALAMM defined in the [introductory vignette](https://docs.ropensci.org/galamm/articles/galamm.html), and for simplicity assuming we use canonical link functions, we have the response model
...
Mixed response models can be combined with heteroscedastic residuals, which are described in the vignette on [linear mixed models with heteroscedastic residuals](https://docs.ropensci.org/galamm/articles/lmm_heteroscedastic.html). However, some care is needed in this case. We illustrate with the dataset `mresp_hsced`, whose first few lines are shown below:
...
#> Warning: the 'nobars' function has moved to the reformulas package. Please update your imports, or ask an upstream
#> package maintainer to do so.
